...
//    return [a[0] + b[0], a[1] + b[1]];
    return (FFT::COMPLEX_NUMBER){ a.real + b.real, a.imag + b.imag };
};
...
FFT::COMPLEX_NUMBER FFT::exponent(float k, float N) {
    ...
    return (FFT::COMPLEX_NUMBER){ cosf(x), sinf(x) };
...
void FFT::ifft(COMPLEX_NUMBER * input, int size, COMPLEX_NUMBER * output) {
    ...
    for(int i = 0; i < size; i++) {
        ...
        csignal[i]= (FFT::COMPLEX_NUMBER){ input[i].imag, input[i].real };
    }
    ...
    for(int j=0;  j < size; j++){
        output[j] = (FFT::COMPLEX_NUMBER){ ps[j].imag / size, ps[j].real / size };
    }
    //        return res;
}
