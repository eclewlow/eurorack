...
void EffectManager::setDepth(float depth) { user_settings.settings_ptr()->fx_depth = std::clamp(depth, 0.0f, 1.0f); }

int EffectManager::getScale() { return user_settings.settings_ptr()->fx_scale; }
void EffectManager::setScale(int scale) { user_settings.settings_ptr()->fx_scale = std::clamp<int>(scale, 0, 100); }

int EffectManager::getRange() { return user_settings.settings_ptr()->fx_range; }
void EffectManager::setRange(int range) { user_settings.settings_ptr()->fx_range = std::clamp<int>(range, 1, 10); }
