...
#include "waves/cv_reader_channel.h"
#include "waves/note_filter.h"
#include "waves/ramp/ramp_divider.h"
#include "waves/ramp/ramp_extractor.h"
#include "waves/random/distributions.h"
#include "waves/random/output_channel.h"
#include "waves/random/random_generator.h"
#include "waves/random/random_sequence.h"
...
#include "waves/test/fixtures.h"
#include "waves/test/ramp_checker.h"
...
void TestBetaDistribution() {
  ...
  // import numpy
  // import pylab
  // data = numpy.loadtxt('waves_histograms.txt')
  // n = 0
  // for i in xrange(9):
  //   for j in xrange(13):
  ...
  //     n += 1
  // pylab.show()
  FILE* fp = fopen("waves_histograms.txt", "w");
  for (int i = 0; i < 9; ++i) {
    for (int j = 0; j < 13; ++j) {
...
void TestQuantizer() {
 ...
 //  import pylab
 //
  // data = numpy.loadtxt('waves_quantizer.txt')
  ...
  // pylab.show()
  FILE* fp = fopen("waves_quantizer.txt", "w");
...
void TestQuantizerNoise() {
  ...
  // import pylab
  //
  // data = numpy.loadtxt('waves_quantizer_hysteresis.txt')
  // pylab.plot(data)
  // pylab.show()
  FILE* fp = fopen("waves_quantizer_hysteresis.txt", "w");
...
void TestRampExtractorClockBug() {
  ...
  wav_writer.Open("waves_ramp_extractor_clock_bug.wav");
...
void TestRampExtractorPause() {
  ...
  wav_writer.Open("waves_ramp_pause.wav");
...
void TestOutputChannel() {
  ...
  wav_writer.Open("waves_random_voltage.wav");
...
void TestXYGeneratorASR() {
  WavWriter wav_writer(4, ::kSampleRate, 10);
  wav_writer.Open("waves_xy_asr.wav");
...
void TestTGeneratorSuperFastClock() {
  WavWriter wav_writer(6, ::kSampleRate, 10);
  wav_writer.Open("waves_t_super_fast.wav");
...
void TestNoteFilter() {
  ...
  FILE* fp = fopen("waves_denoising.txt", "w");
  FILE* fp2 = fopen("waves_denoising_error.txt", "w");
...
void TestCVChannel() {
  ...
  FILE* fp = fopen("waves_cv_channel.txt", "w");
...
void TestClockSourceChange() {
  WavWriter wav_writer(4, ::kSampleRate, 10);
  wav_writer.Open("waves_clock_source_change.wav");
...
int main(void) {
  ...
  // Ramp tests.
  // TestRampExtractor(FRIENDLY_PATTERNS, "waves_ramp_extractor_friendly.wav");
  // TestRampExtractor(TRICKY_PATTERNS, "waves_ramp_extractor_tricky.wav");
  // TestRampExtractor(FAST_PATTERNS, "waves_ramp_extractor_fast.wav");
  // TestRampDivider(TRICKY_PATTERNS, "waves_ramp_divider_tricky.wav");
