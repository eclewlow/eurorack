    ...
    if(key == LEFT_ENCODER_CCW) {
        switch(state_) {
            ...
            case MANAGE_MENU_MOVE_WAVETABLE: {
                int16_t target_wavetable = CLAMP<int16_t>(wavetable_ - 1, 0, USER_WAVETABLE_COUNT + FACTORY_WAVETABLE_COUNT - 1);
            ...
            case MANAGE_MENU_MOVE_FRAME: {
                int16_t target_frame = CLAMP<int16_t>(frame_ - 1, 0, 15);
            ...
            case MANAGE_MENU_FRAME_OPTIONS:
                if(false) //storage.GetWavetable(wavetable_)->factory_preset)
                {
                    ...
                    setOptionSelected(CLAMP<int8_t>(option_selected_ - 1, MANAGE_MENU_EDIT, MANAGE_MENU_DELETE));
            ...
            case MANAGE_MENU_CONFIRM:
                setOptionSelected(CLAMP<int8_t>(option_selected_ - 1, MANAGE_MENU_NO, MANAGE_MENU_YES));
                break;
            default:
                break;
        }
